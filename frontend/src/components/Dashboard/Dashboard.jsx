...
export function Dashboard({ classes, todayClasses, courses, userStats, setView, openEdit, openCreate, userRole = 'admin' }) {
  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-700">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 bg-white px-4 py-3 rounded-2xl shadow-sm border border-slate-100 text-slate-500 font-bold text-sm">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        <section className="lg:col-span-2 bg-white rounded-[2.5rem] border border-slate-100 p-8 shadow-xl shadow-slate-200/40">
          <div className="flex items-center justify-between mb-8">
            ...
            <button className="bg-indigo-50 text-indigo-700 px-4 py-2 rounded-xl text-xs font-bold hover:bg-indigo-100 transition-all flex items-center gap-2 cursor-pointer" onClick={() => setView('calendar')}>
              Full Calendar
          ...
          <div className="space-y-4">
            {todayClasses.length > 0 ? todayClasses.map(c => (
              <ClassItem key={c.id} item={c} onEdit={openEdit} userRole={userRole} />
            )) : (
              <div className="text-center py-20 bg-slate-50/50 rounded-[2rem] border-2 border-dashed border-slate-100">
                ...
                {userRole === 'admin' && (
                  <button onClick={() => openCreate()} className="mt-4 text-indigo-600 font-black text-sm hover:underline underline-offset-4 cursor-pointer">Schedule one now</button>
                )}
              </div>
            )}
          </div>
            ...
            {upcomingClasses.map(c => (
              <div key={c.id} className="flex gap-5 group cursor-pointer" onClick={() => openEdit(c)}>
                ...
                <div className="flex-1 min-w-0">
                  <h4 className="font-bold text-slate-800 truncate group-hover:text-indigo-600 transition-colors">{c.topic_name}</h4>
                  <div className="flex items-center gap-3 mt-1">
                    <span className="text-xs font-bold text-slate-400 flex items-center gap-1">
                       <Clock size={12} /> {c.start_time}
                    </span>
                    <span className="w-1 h-1 bg-slate-200 rounded-full"></span>
                    <span className="text-xs font-bold text-indigo-500/70 truncate">Course: {c.course_name}</span>
                  </div>
                </div>
              </div>
            ))}
